# scan for sturgeon (Atlantic and short nose) and spp (all species) records and join only those records
# source("2-pseg_datafix.R")
library(sf)
sf::sf_use_s2(FALSE)
library(dplyr)
spp_of_interest <- "BLACK DRUM"
...
beach_locs <- read.csv(file.path(root, "pseg/PSEG 2025 Beach Seine Locations.csv"))
trawl_locs <- read.csv(file.path(root, "pseg/PSEG Bottom Trawl Locations.csv"))[, 1:3]
strata <- read_sf(file.path(root, "pseg/pseg_strata.gpkg"))
#########

data_names <- gsub("(Level [0-9]_[A-Za-z ]* [0-9]{4}).*", "\\1", names(pseg))
gears <- unique(sapply(data_names, function(y) gsub("Level [0-9]_([A-Za-z ]*) [0-9]{4}.*", "\\1", y)))
samples <- sapply(gears, function(x) {
  sum(sapply(pseg[which(grepl(paste0("Level 1_", x), data_names))], nrow))
})

get_spp <- function(x) {
  if ("COMMON_NAME" %in% names(x)) {
    x <- x[grep(spp_of_interest, x$COMMON_NAME), ] # "STRIPED BASS STURGEON|spp
  } else {
    x <- data.frame()
  }
  return(x)
}
spp <- lapply(pseg, get_spp)
spp <- spp[sapply(spp, nrow) > 0]

coords <- apply(beach_locs[, 2:3], 2, strsplit, " ")
...
beach_locs <- as.data.frame(deg + min)
beach_locs$sta <- 1:nrow(beach_locs)
beach_locs$lng <- beach_locs$lng * -1
...
lvl2 <- Map(function(x, y) {
  ...
  if (gsub("(Level [0-9])_[A-Za-z ]* [0-9]{4}.*", "\\1", y) == "Level 2") {
    lvl1 <- data_names[which(grepl(paste0("Level 1_", gear), data_names))]
    ...
    level1 <- lvl1[index]
    print(nrow(x))
    ref <- pseg[[which(data_names == level1)]]
    ...
    zerofill$COMMON_NAME <- spp_of_interest
    ...
    if ("GRID" %in% names(out)) { # Bottom Trawl, Ichthyoplankton, Pelagic Trawl
      ...
      out <- merge(out, beach_locs, by.x = "LOC_CODE", by.y = "sta", all.x = T)
      out$LOC_CODE <- NULL
    }
    ...
    out$TOTAL_COUNTED <- as.integer(out$TOTAL_COUNTED)
      ...
      select(SAMPLE_ID, COMMON_NAME, TOTAL_COUNTED) %>%
      group_by(SAMPLE_ID, COMMON_NAME) %>% # I broke this, it must be changing this here...check type is char?
...
# }
# checkcode <- lapply(pseg, findcode)
# checkcode <- checkcode[sapply(checkcode, nrow)>0]
sngs <- which(datatabs[[1]]$GPS_Group_ID_lookup$Value == "SNGS")
datatabs[[1]]$GPS_Group_ID_lookup[sngs, ]$Value <- datatabs[[1]]$GPS_Group_ID_lookup[sngs, ]$site_id
...
# Maybe also total number of stations conducted for the year (whether sturgeon/spp were caught or not), to get an estimate of overall interaction frequency
# [[1]]$Weight_Measurement_Code_lookup

# "MAXIMUM_WEIGHT", "MINIMUM_WEIGHT",
lvl2s <- lapply(lvl2, function(x) {
  x <- x[, names(x)[!names(x) %in% c(
    "END_LAT_DD", "END_LAT_MM", "END_LON_DD",
    "END_LON_MM"
  )]]
  return(x)
})

out_data <- lapply(1:length(lvl2s), function(a) {
...
out_data <- out_data[sapply(out_data, nrow) > 0]
lapply(out_data, function(d) {
  d <- d[, colSums(!is.na(d)) > 0]
  write.csv(d, file = file.path(out, paste0(spp_of_interest, "-catch_", d$GEAR_TYPE[1], ".csv")), row.names = F)
})

# out_data <- bind_rows(lvl2s)
# out_data <- out_data[, c(
...
#  "filename"
# )]
# lvl2data <- out_data[order(out_data$GEAR_TYPE, out_data$COLLECTION_DATE), ]
...
out_data_lvl3 <- lapply(lvl3, function(x) {
  x <- x[, names(x)[names(x) %in% c(
    "SAMPLE_ID", "CNT_LVL3",
...
out_data_lvl3 <- bind_rows(out_data_lvl3)
# out_data[is.na(out_data$LENGTH), ]$LENGTH <- out_data[is.na(out_data$LENGTH), ]$INDIVIDUAL_LENGTH
# out_data$INDIVIDUAL_LENGTH <- NULL

mismatch <- out_data_lvl3[out_data_lvl3$expand < 1 & !is.na(out_data_lvl3$expand), ]
good <- out_data_lvl3[out_data_lvl3$expand >= 1 & !is.na(out_data_lvl3$expand), ]
write.csv(out_data_lvl3, file.path(out, paste0(spp_of_interest, "_length.csv")), row.names = F)
