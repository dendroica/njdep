...
LFD <- function(mypath, myspp, area = "ALL", cruise = "ALL", outdir) {
  ...
  YMEANSALL <- ABUNDALL %>%
    group_by(YEAR) %>%
    summarise(
      TOTALN = sum(NUMBER, na.rm = TRUE), SAMPLES = n(),
      .groups = "drop"
    ) %>%
    left_join(AMALL, by = "YEAR") %>%
  ...
  AM4 <- ABUND %>%
    group_by(YEAR) %>%
    summarise(
      TOTALN = sum(NUMBER, na.rm = TRUE),
      SAMPLES = n(),
      .groups = "drop"
    ) %>%
    left_join(AM, by = "YEAR") %>%
    mutate(NMEAN = NM2, SNVE = sqrt(SNV2)) %>%
    select(-c(NM2, SNV2))
  ...
  STAM2 <- ABUND %>%
    group_by(STRATUM) %>%
    summarise(
      TN = sum(NUMBER, na.rm = TRUE), NM = mean(NUMBER, na.rm = TRUE),
  ...
  LENG2 <- LENG2 %>%
    group_by(YEAR) %>%
    complete(
      LENGTH = full_seq(c(1, max(LENGTH)), period = 1),
      fill = list(value = 0)
    ) %>%
    ungroup() %>%
  ...
  lengy <- lengy %>%
    group_by(YEAR, STRATUM, LENGA, FACTOR) %>%
    summarise(SUM = sum(FREQUENCY, na.rm = TRUE), .groups = "drop") %>%
    left_join(SAMPLES, by = c("YEAR", "STRATUM")) %>%
    filter(!is.na(SUM)) %>%
    mutate(MEAN1 = FACTOR * (SUM / COUNT)) %>%
    group_by(YEAR, LENGA) %>%
    ...
    group_by(YEAR) %>%
    complete(
      LENGA = full_seq(c(1, max(LENGA)), period = 1),
      fill = list(value = 0)
    ) %>%
