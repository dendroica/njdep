...
ShowZeros <- function(XX) {
  ...
  library(zoo)
    ...
    #Taken from: https://urldefense.com/v3/__http://www.r-bloggers.com/controlling-heatmap-colors-with-ggplot2/__;!!J30X0ZrnC1oQtbA!Md54Nx6EzFqfssAJ5p19QKZCGZ9RfeOq29uI03xsgyjN5U31efYQJdSvMwfzg9dekg5KtnQ2vHgimDgZPxdd5ZNvzjDO$ 
    ...
    ## use https://urldefense.com/v3/__http://colorbrewer2.org/__;!!J30X0ZrnC1oQtbA!Md54Nx6EzFqfssAJ5p19QKZCGZ9RfeOq29uI03xsgyjN5U31efYQJdSvMwfzg9dekg5KtnQ2vHgimDgZPxdd5YnrPsS6$  to find optimal divergent color palette (or set own)
...
# https://urldefense.com/v3/__https://haakonbakka.bitbucket.io/btopic105.html__;!!J30X0ZrnC1oQtbA!Md54Nx6EzFqfssAJ5p19QKZCGZ9RfeOq29uI03xsgyjN5U31efYQJdSvMwfzg9dekg5KtnQ2vHgimDgZPxdd5W0w-WeQ$ 
...
# Plot the smoothers
MySmootherPlot <- function(Model, Stack, Tag, Data4Prediction, 
                           CovariateName, MyXLab, MyYLab){
  Index    <- inla.stack.index(Stack, tag = Tag)$data
  Smoother <- Model$summary.fitted.values[Index, c(1,3,5)]  
  Smoother <-  plyr::rename(Smoother, 
                            c("mean" = "mu", 
                              "0.025quant" = "selo",
                              "0.975quant" = "seup"))
  Smoother$X <- Data4Prediction[,CovariateName]
  
  pp <- ggplot()
  pp <- pp + xlab(MyXLab) + ylab(MyYLab)
  pp <- pp + theme(text = element_text(size = 10))
  pp <- pp + geom_line(data = Smoother,  aes(x = X, y = mu))
  pp <- pp + geom_ribbon(data = Smoother, 
                         aes(x = X, 
                             ymax = seup, 
                             ymin = selo),
                         alpha = 0.6)
